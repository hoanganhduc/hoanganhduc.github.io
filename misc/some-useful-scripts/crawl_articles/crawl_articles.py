...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        '--data-sources',
        type=str,
        help="Comma-separated list of additional data sources (URLs or file paths) to fetch existing titles from"
    )
    ...
    parser.add_argument(
        ...
        '--limit',
        type=int,
        help="Limit on the number of resulting articles to fetch from each database (default: 20)"
    )
...
def validate_arguments(args):
    ...
    """
    # Check if the number of years is a non-negative integer
    if args.years is not None and args.years < 0:
        raise ValueError("The number of years must be a non-negative integer.")

    # Check if the limit on the number of articles is a positive integer
    if args.limit is not None and args.limit <= 0:
        raise ValueError("The limit on the number of articles must be a positive integer.")
    ...
    if args.output_formats:
        ...
        specified_formats = set(args.output_formats.split(','))
        if not specified_formats.issubset(valid_formats):
            raise ValueError("Invalid output format specified. Valid options are 'bib', 'html', 'pdf', 'all'.")
...
def print_welcome_message():
    ...
    print("You can also generate a requirements.txt file, clean up generated files, and enable debug mode for detailed logs.")
...
def main():
    ...
    2. Prints default keywords if the corresponding argument is provided.
    3. Generates a requirements file if the corresponding argument is provided.
    4. Uses default settings or user-provided settings to fetch keywords, years, databases, and additional sources.
    5. Fetches additional data sources based on user arguments.
    6. Combines default and additional data sources.
    7. Fetches existing article titles from all data sources.
    8. Fetches all articles based on selected keywords, existing titles, years, and databases.
    9. Prints the total number of articles found.
    10. Saves the fetched articles to files.
    11. Prints environment and resource usage information.
    """
    ...
    if args.default:
        # Use default settings if the corresponding argument is provided
        all_keywords, selected_keywords, years, databases, additional_sources, limit, output_format = use_default_settings()
    else:
        # Use user-provided settings if the corresponding argument is not provided
    ...
    # Fetch existing article titles from all data sources
    existing_titles = fetch_existing_titles(all_data_sources)
    
    # Fetch all articles based on selected keywords, existing titles, years, and databases
    ...
    # Save the fetched articles to files
