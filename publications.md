...
* If you have an arXiv number of an article, say `XXXX.YYYYY`, then
  * The link `https://arxiv.org/src/XXXX.YYYYY` allows you to download the TeX source (if available) in `*.tar.gz` format.
  * The link `https://arxiv.org/pdf/XXXX.YYYYY.pdf` allows you to view the PDF version.
  * The link `https://ar5iv.org/abs/XXXX.YYYYY` allows you to view the HTML version generated by [LaTeXML](http://dlmf.nist.gov/LaTeXML/) (if available).
