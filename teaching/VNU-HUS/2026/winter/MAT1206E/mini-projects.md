...
This page explains how to prepare and register a mini-project topic for "Introduction to Artificial Intelligence (VNU-HUS MAT1206E)" during Semester 1 of the 2026-2027 academic year. The course topic-board link will be added here after the board is created and verified.
...
**Preparation is open; registration links are not yet published here.** The Classroom50 `final-project` acceptance link is scheduled for publication on **September 11, 2026 at 13:00 ICT (UTC+7)**. The course topic board will be linked here after it is created and verified. Until then, form your group, read the guide, and prepare your ideas; do not open a topic-registration issue in the template repository.

1. **Form the group and choose one founder.** Agree on one to five students from the same course and verify everyone's GitHub username. When the course topic board is available, review existing proposals before settling on a precise problem.
...
6. **Register through one course topic-board issue.** Once the board is published, search for your exact group-repository URL and the founder's GitHub username. Reuse an existing group issue; repair an incomplete issue rather than opening another, and ask staff if the canonical issue is unclear. If no group issue exists, the founder chooses **Issues → New issue → Project topic proposal**, completes every field, and links the exact proposal commit. Follow the [completed issue example](https://github.com/VNU-HUS/introai-final-project-template/blob/main/examples/topic-proposal/topic-issue.example.md).
...
The course topic board is not yet published. Its verified link will appear here; the canonical issues there will hold topic records and updates, rather than a second registration list on this page.
